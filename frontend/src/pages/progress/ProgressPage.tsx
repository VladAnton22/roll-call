import { useTechniqueRatings } from "../../hooks/useTechniqueRatings.ts";
import { useSessionLog } from "../../hooks/useSessionLog.ts";
import CategoryRadarChart from "./CategoryRadarChart.tsx";
import ProgressSummary from "./ProgressSummary.tsx";
import PracticeVsProficiency from "./PracticeVsProficiency.tsx";

export default function ProgressPage() {
  const { ratings } = useTechniqueRatings();
  const { sessions, sessionsThisWeek } = useSessionLog();
  return (
    <main className="max-w-2xl mx-auto px-4 pb-24 pt-6 space-y-3">
      <div className="mb-6">
        <h1 className="text-2xl font-black tracking-tight text-content-primary mb-1">
          Your Progress
        </h1>
      </div>
      <ProgressSummary ratings={ratings} sessionsThisWeek={sessionsThisWeek} />
      <CategoryRadarChart ratings={ratings} />
      <PracticeVsProficiency ratings={ratings} sessions={sessions} />
    </main>
  );
}